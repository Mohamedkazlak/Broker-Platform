...
function encryptRegistrationPayload({ formData, package: pkg, domain, domainFields }) {
 ...
 * Public Instapay transfer details for the QR / account page.
 */
 ...
 *   Stores encrypted registration payload + receipt. No auth.users/brokers yet.
 *
 ...
 * Public claim-token poll for the pending screen (no session required).
 * When approved, returns a one-time session so the client can continue onboarding.
 */
export const getStatus = async (req, res, next) => {
  try {
    ...
    if (submission.status === "approved") {
      ...
      // One-time session handoff for deferred signups (password still encrypted).
      ...
      } else if (submission.broker_id) {
        // Upgrade path — client already has a session; nothing to hand off.
      }
    }
 ...
 * List Instapay submissions (default: pending_review).
 */
  ...
  try {
    ...
    if (action === "reject") {
      ...
      const updated = await instapayModel.update(submission.id, {
        ...
        reviewed_at: now,
        // Drop encrypted credentials; email/domain soft-holds release via status.
        registration_payload: null,
      });
    ...
    if (!broker) {
      ...
      // Keep passwordEnc until the pending page claims the session once.
      await instapayModel.update(submission.id, {
        broker_id: broker.id,
        status: "approved",
