...
import { Globe, Check, ArrowRight, Building2, Star, Zap } from "lucide-react";
...
const planDefs: PlanDef[] = [
  {
    ...
    colors: {
      border:
        "border-border hover:border-blue-300 border-t-4 border-t-blue-500",
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
            {planDefs.map((plan, index) => {
              const features = t(`plans.${plan.key}.features`, {
                returnObjects: true,
              }) as string[];
              ...
              return (
                <Card
                  key={plan.key}
                  className={`relative overflow-hidden transition-all duration-300 hover:shadow-card-hover animate-fade-in opacity-0 ${plan.colors.border} ${
                ...
                >
                  {plan.highlighted && (
                    <div
                      className={`absolute top-0 inset-x-0 text-center py-1.5 text-sm font-semibold ${plan.colors.badge}`}
                    >
                      {t("mostPopular")}
                    </div>
                  )}
